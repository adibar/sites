...
(function($)
{
	...
	$.Redactor.opts = {
			...
			langs: {
				en: {
					...
					edit: 'Edit',
					link_buttonshape: 'Design link as shaped button',
					custombuttonColor: 'Text color',
					custombuttonBackground: 'Background color',
					custombuttonRoundedCorners: 'Rounded corners: ',
					custombuttonExample: '- Example -',
					tblborder: 'Border Width (0 = No border)'
				}
			}
	};
	...
	Redactor.fn = $.Redactor.prototype = {
			...
			if (scrollTop > boxTop)
			{
				...
				if (this.opts.toolbarFixedTarget === document)
				{
					this.$toolbar.css({
						...
						width: width,
						zIndex: 15,
						top: this.opts.toolbarFixedTopOffset + 5 + 'px',
						left: left
					});
				}
				else
				{
					this.$toolbar.css({
						...
						width: width,
						zIndex: 15,
						top: (this.opts.toolbarFixedTopOffset + scrollTop) + 5 + 'px',
						left: 0
					});
				}
		...
		// ALIGNMENT
		//O.L - Added the 'direction' value for align correctly for each align type. 06.07.14
		alignmentLeft: function()
		{
			//O.L - Added 'left' to settings - was empty and didnt align. 06.07.14
			this.alignmentSet('left', 'ltr', 'JustifyLeft');
		},
		alignmentRight: function()
		{
			this.alignmentSet('right', 'rtl', 'JustifyRight');
		},
		...
		//O.L - Added the 'direction' value for align correctly for each align type. 06.07.14
		alignmentSet: function(type, direction, cmd)
		{
			this.bufferSet();
		...
		tableInsert: function()
		{
			...
			//O.L: Set border with value
			var border = $('#redactor_table_border').val();
			if(border>0 && border<10){
				borderStyle = "style='border: " + border +"px solid; padding: 5px;'";
			}else{
				borderStyle = "";
			}
			// -- end of edit
			...
			var rows = $('#redactor_table_rows').val(),
				...
				$table = $('<table id="table' + tableId + '" style="width: 100%; max-width:100%;"><tbody></tbody></table>'),
				i, $row, z, $column;
			...
			for (i = 0; i < rows; i++)
			{
				...
				for (z = 0; z < columns; z++)
				{
					...
					//O.L: Insert border value
					$column = $('<td ' + borderStyle + '><br></td>');
					// -- end of edit
		...
		linkShow: function()
		{
			...
			var callback = $.proxy(function()
			{
				...
				var sel = this.getSelection();
				var url = '', text = '', target = ''; 
				//O.L: Adding variables to function (search 'Add custom shape link' for more description)
				//Ref code: BTN_ADD_EXISTED_VAL
				isButton = 0; a_color='';a_bgcolor='';a_radius='';a_class='';
				...
				if (elem && elem.tagName === 'A')
				{
					...
					//O.L: Set variables from exist button (search 'Add custom shape link' for more description)
					//Ref code: BTN_VAR_SET
					a_color = $(elem).css("color");
					a_bgcolor = $(elem).css("background-color");
					a_radius = $(elem).attr("data-radius");
					a_class = $(elem).attr("class");
					//-- end of editing
				...
				//O.L: Initialize button style options (search 'Add custom shape link' for more description)
				//Ref code: BTN_STYLE_INIT
				if (a_class === 'f_btn1'){
					$('#redactor_link_buttonshape').prop('checked', true);
					$('.button_shape_options, .btn_example').show();
					$("#btn_fontcolor").val(rgb2hex(a_color));
					$("#btn_fontbg").val(rgb2hex(a_bgcolor));

					$("#btn_radius").attr("data-val", a_radius);
					$("#btn_radius_val").html(a_radius);
				}
				// -- end of editing
			...
			/* ---------------------- Add custom shape link ----------------------
			//	By: Oren Lande						Created: 13.2.14			//
			//	Description: Initilizing custom button settings					//
			//				 Find the changes by CTRL+F the referance code		//
			//				 Reference code	written in brackets.				//
			//	Relevant function: 												//
			//		--------- Basic initilizing			---------				//
			//		initRedactorAddons() (../../../common.js)					//
			//		HTML Code (BTN_SET_HTML)									//
			//																	//
			//		--------- When editing exist button	---------				//
			//		Set variables to exist button (BTN_VAR_SET)					//
			//		Set style when editing button (BTN_STYLE_INIT)				//
			//		Create variables from existing button (BTN_ADD_EXISTED_VAL)	//
			//		Insert variables into exist button (BTN_EDIT_STYLE)			//
			//																	//
			//		--------- When adding new button    ---------				//
			//		Set the variables when new button created (BTN_NEW_INIT)	//
			//		Set the HTML code of button/link (BTN_HTML_CODE)			//
			//		Create new variables for button (BTN_ADD_NEW_VAL)			//
			//		Insert variables into new button (BTN_ADD_STYLE)			//
			//																	//
			//	Changelog:		 								Change by:		//
			//	06.07.14 - Updated to redactor 9.25				Oren Lande		//
			//	06.07.14 - Added instructions to missed lines	Oren Lande		//
			----------------------------------------------------------------------*/
			initRedactorAddons();
		...
		linkProcess: function()
		{
			...
			else if (link.search('#') != 0)
			{
				...
				//O.L: When creating new button, initialize button style options (search 'Add custom shape link' for more description)
				//Ref code: BTN_NEW_INIT
				if ($('#redactor_link_buttonshape').prop('checked'))
				{
					isButton = 1;
					btnFontColor = $("#btn_fontcolor").val();
					btnBGColor = $("#btn_fontbg").val();
					btnRadiusVal = $("#btn_radius").attr("data-val");
					btnRadius = $("#btn_radius").attr("data-val");
					if(btnRadius != "" && btnRadius != "0"){
						btnRadius = ' border-radius:'+btnRadius+'px;';
					}else{
						btnRadius = ""
					}
					//button styles
					btnStyle = "color:"+btnFontColor+"; background-color:"+btnBGColor+";"+btnRadius;
				}else{
					isButton = 0;
				}
				// -- end of editing
			...
			//O.L: Setting the HTML code of the new button/link (search 'Add custom shape link' for more description)
			//Ref code: BTN_HTML_CODE
			if(isButton == 1){
				this.linkInsert('<a href="' + link + '"' + target + '>' + text + '</a>' + extra, $.trim(text), link, targetBlank, "f_btn1", btnStyle, btnRadiusVal);
			}else{
		...
		//O.L: Adding variables to function (search 'Add custom shape link' for more description)
		//Ref code: BTN_ADD_NEW_VAL
		linkInsert: function (a, text, link, target, a_class, a_style, a_radius)
		{
			this.selectionRestore();
			...
			if (text !== '')
			{
				if (this.insert_link_node)
				{
					...
					//O.L: Insert the new variables into exist button (search 'Add custom shape link' for more description)
					//Ref code: BTN_EDIT_STYLE
					if(a_class !== ''){$(this.insert_link_node).addClass(a_class);}
					if(a_style !== ''){$(this.insert_link_node).attr("style",a_style);}
					if(a_radius !== ''){$(this.insert_link_node).attr("data-radius",a_radius);}
					this.sync();
					// -- end of editing
				}
				else
				{
					...
					{
						//O.L: Insert the new variables into NEW button (search 'Add custom shape link' for more description)
						//Ref code: BTN_ADD_STYLE
						if(a_class !== ''){$(this).addClass(a_class);}
						if(a_style !== ''){$(this).attr("style",a_style);}
						if(a_radius !== ''){$(this).attr("data-radius",a_radius);}
						// -- end of editing
		...
		modalTemplatesInit: function()
		{
			$.extend( this.opts,
			{
				...
				+ '<section id="redactor-modal-link-insert">'
					...
					+ '<label><input type="checkbox" id="redactor_link_blank"> ' + this.opts.curLang.link_new_tab + '</label>'
					//O.L: Set the html code of options (search 'Add custom shape link' for more description)
					//Ref code: BTN_SET_HTML
					+ '<label style="display:inline;"><input type="checkbox" id="redactor_link_buttonshape">' + this.opts.curLang.link_buttonshape + '</label>'
					+ '<table class="button_shape_options" style="display:none;">'
						+ '<tr><td>'
							+ '<label>'+ this.opts.curLang.custombuttonColor +'</label>'
							+ '<input type="text" id="btn_fontcolor" class="color-picker" size="7" />'
						+ '</td>'
						+ '<td>'
							+ '<label>'+ this.opts.curLang.custombuttonBackground +'</label>'
							+ '<input type="text" id="btn_fontbg" class="color-picker" size="7" />'
						+ '</td>'
						+ '<td>'
							+ '<label style="position:relative; bottom:8px; margin-bottom:5px;">'+ this.opts.curLang.custombuttonRoundedCorners +'<span id="btn_radius_val"></label>'
							+ '<div id="btn_radius" class="range_slider" data-val=""></div>'
						+ '</td></tr>'
					+ '</table>'
					+ '<div class="btn_example" style="display:none;">'
						+ '<span>'+ this.opts.curLang.custombuttonExample +'</span>'
					+ '</div>'
				...
				+ '<section id="redactor-modal-table-insert">'
					...
					//O.L: Set HTML code for border width options
					+ '<label>' + this.opts.curLang.tblborder + '</label>'
					+ '<input type="text" size="5" value="1" id="redactor_table_border" />'
					// -- end of edit
				+ '</section>'
				+ '<footer>'
					+ '<button class="redactor_modal_btn redactor_btn_modal_close">' + this.opts.curLang.cancel + '</button>'
