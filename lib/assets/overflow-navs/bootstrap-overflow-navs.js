...
+function ($) { "use strict";
	...
	$.fn.overflowNavs = function(options) {
		...
		if (! dropdown.length) {
			dropdown = $('<li id="overflow-nav" class="overflow-nav dropdown"></li>');
			dropdown.append($('<a class="dropdown-toggle" data-toggle="dropdown" href="#">' + options.more + '<b class="caret"></b></a>'));
			dropdown.append($('<ul class="dropdown-menu pull-right" style=""></ul>'));
		}
        ...
        // adi baron remove all dropdown items if mobile
        var is_mobile = $('.navbar-toggle').is(":visible");
        if (is_mobile) {
		...
		var backgound_str = $('.wrapper-addons').css('background-color');
		if (backgound_str) {
			var dropdown_background_color = rgba_to_rgb(backgound_str);
			$('.dropdown-menu').css( {'background-color': dropdown_background_color, } );
		}
...
// remove alpha channel form rgba
function rgba_to_rgb(rgba_str) {
	colorsOnly = rgba_str.substring(rgba_str.indexOf('(') + 1, rgba_str.lastIndexOf(')')).split(/,\s*/),
  components = {};
